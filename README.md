...
1. 動画を **mp4形式（H.264・プログレッシブ）** で用意する（1ファイル100MB以下を推奨）
...
> **⚠️ インターレース動画は iPhone で再生できません**
>
> ビデオカメラで撮った **1080i（インターレース）** のままの動画は、PCのブラウザでは
> 再生できても **iPhone / iPad では再生できません**（iOSのデコーダが非対応）。
> 特に `1440x1080` や `1920x1080i` のカメラ元データは要注意です。
>
> 書き出すときに必ず **「プログレッシブ」／「インターレース解除」** を選んでください。
> macOS だけで直す場合は次のコマンドで再エンコードできます（1280x720・プログレッシブ）。
>
> ```sh
> avconvert --source 元動画.mp4 --preset Preset1280x720 --output 新動画.mp4 --multiPass
> ```
>
> ※ `Preset1920x1080` は「元が1080以下」だと再エンコードせずそのまま通してしまうため、
> インターレースが直りません。必ず `Preset1280x720` など解像度が下がるプリセットを使うこと。
...
| `prayer.mp4` | 夏祭りの一日「祈願｜安全祈願」の動画 |
| `children-performance-1.mp4` | 夏祭りの一日「出発｜山車の巡回スタート」の動画（1本目） |
| `children-performance-2.mp4` | 夏祭りの一日「出発｜山車の巡回スタート」の動画（2本目） |
| `parade-1.mp4` | 夏祭りの一日「巡回中｜地域のみんなと交流」の動画（1本目） |
| `parade-2.mp4` | 夏祭りの一日「巡回中｜地域のみんなと交流」の動画（2本目） |
| `joint-performance.mp4` | 夏祭りの一日「共演｜逆西地区との共演」の動画 |
...
- 1つのステップに写真・動画を複数入れると、**スワイプ（横スクロール）で切り替えられるカルーセル**になります。
  `src/data/festivalData.ts` の各ステップの `media` 配列に並べた順で表示されます。
